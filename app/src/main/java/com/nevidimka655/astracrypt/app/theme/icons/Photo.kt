...
@Composable
private fun default(): VectorPainter = with(MaterialTheme.extendedColorScheme.green) {
    rememberVectorPainter(photo(background = colorContainer, foreground = onColorContainer))
}

@Composable
private fun alt(): VectorPainter = with(MaterialTheme.colorScheme) {
    rememberVectorPainter(photo(background = onSurfaceVariant, foreground = surface))
}
